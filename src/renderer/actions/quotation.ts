...
export const SORT_QUOTATIONS_WITH_COLLAPSE_CHACHED = 'SORT_QUOTATIONS_WITH_COLLAPSE_CHACHED';
...
export function getFavoriteQuotationMap() {
  return Utils.GetStorage(CONST.STORAGE.FAVORITE_QUOTATION_MAP, {} as { [index: string]: boolean });
}
...
export function loadQuotations() {
  return async (dispatch: Dispatch, getState: GetState) => {
    try {
      ...
      batch(() => {
        dispatch({ type: SORT_QUOTATIONS_WITH_COLLAPSE_CHACHED, payload: quotations });
